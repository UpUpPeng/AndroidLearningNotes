...
```java
// JVM 字节码指令（基于栈）
0: iload_1	// 从局部变量1号槽位读取变量x的值到操作数栈
1: iload_2	// 从局部变量2号槽位读取变量y的值到操作数栈
2: iadd		// 栈顶两个元素相加并保存到操作数栈(x+y)
3: iload_1	// 从局部变量1号槽位读取变量x的值到操作数栈
4: iload_2	// 从局部变量2号槽位读取变量y的值到操作数栈
5: isub		// 栈顶两个元素相减并保存到操作数栈(x-y)
6: imul		// 栈顶两个元素相乘并保存到操作数栈
